...
-- 2023-02-10,CHelbling,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment06DB_CHelbling')
	 Begin 
	  Alter Database [Assignment06DB_CHelbling] set Single_user With Rollback Immediate;
	  Drop Database Assignment06DB_CHelbling;
	 End
	Create Database Assignment06DB_CHelbling;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment06DB_CHelbling;
...
AS
SELECT [ManagerName] = m.EmployeeFirstName + ' ' + m.EmployeeLastName
	,[EmployeeName] = e.EmployeeFirstName + ' ' + e.EmployeeLastName
	,[EmployeeID] = e.EmployeeID -- adding the EmployeeID to this view so I can use as a key to link to other views.
FROM Employees AS e
...
INNER JOIN vEmployeesByManager AS vem
ON ve.EmployeeID = vem.EmployeeID;

Go

SELECT CategoryID, CategoryName, ProductID, ProductName, UnitPrice, InventoryID, InventoryDate, [Count], EmployeeID, EmployeeName, ManagerName
